<template>
    <ModalBox :size="'small'" :show="show">
            ...
            <form class="grid grid-cols-1 gap-4 sm:grid-cols-6">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-6 sm:col-span-6">
                    ...
                    <div class="sm:col-span-6">
                        ...
                        <div class="mt-1 rounded-md shadow-sm">
                            <select id="account-type" v-model="form.account_type_id"
                                    v-bind:class="{ 'border-red-500': !validations.account_type_id.valid }"
                                    class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                                <option value=""></option>
                                <option v-for="(accountType, key) in accountTypes"
                                        v-bind:key="'accountType-'+key"
                                        v-bind:value="accountType.id"
                                        v-text="accountType.type"/>
                            </select>
                        </div>
                        <span class="text-red-500 text-sm" v-show="!validations.account_type_id.valid"
                              v-text="validations.account_type_id.message"></span>
                    </div>
                    <div class="sm:col-span-6">
        ...
        <template v-slot:footer>
            ...
            <button @click="addAccount()" type="button"
...
export default {
    ...
    methods: {
        ...
        validate() {
            const {name, iban, account_type_id, description, balance} = this.form;
            ...
            validations.account_type_id.valid = account_type_id !== '';
